...
export const SkillsPageSider = () => {
  ...
  const canAddSyncTarget = Boolean(window.skillsManager?.selectTargetDirectory) && Boolean(window.skillsManager?.addSkillDirectoryTarget);
    ...
    <>
      <section className="min-w-0 rounded-xl border border-border bg-card p-4">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <Button type="button" disabled={!distributionReady || page.isDistributionPreviewLoading} title={syncTitle} aria-label={t("skills.actions.syncCurrentSkillAria")} onClick={page.startSelectedSkillDistribution}>
      ...
      <section className="rounded-xl border border-border bg-card p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <Button type="button" variant="outline" size="sm" disabled={!canAddSyncTarget} title={canAddSyncTarget ? undefined : t("skills.actions.addSyncTargetUnavailable")} onClick={page.addSyncTargetForSelectedSkill}>
        ...
        <div className="mt-3 grid gap-2">
          {targetOptions.map((target) => {
            ...
            return (
              <label key={target.id} className={cn("grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-lg border p-3", checked ? "border-primary/40 bg-primary/5" : "border-border bg-background")}>
                <span className="min-w-0">
                  <strong className="block text-sm">{target.name.startsWith("skills.") ? t(target.name) : target.name}</strong>
                  <span className="block truncate font-mono text-xs text-muted-foreground">{target.path}</span>
                </span>
                <Checkbox
                  checked={checked}
      ...
      <SkillTargetRemovalDialog isExecuting={page.isTargetRemovalExecuting} onClose={page.closeTargetRemovalDialog} onConfirm={page.confirmTargetRemoval} removal={page.pendingTargetRemoval} />
...
const SkillTargetRemovalDialog = ({ isExecuting, onClose, onConfirm, removal }: { isExecuting: boolean; onClose: () => void; onConfirm: (input: { deleteInstalledFiles: boolean; removeTargetPreference: boolean }) => void; removal: SkillsPageState["pendingTargetRemoval"] }) => {
  const { t } = useTranslation();
  ...
  const targetName = removal.targetName.startsWith("skills.") ? t(removal.targetName) : removal.targetName;
      ...
      <DialogPortal>
        ...
        <DialogPopup>
          <div className="mb-5 flex items-start justify-between gap-4">
            ...
            <DialogClose disabled={isExecuting} render={<Button type="button" variant="outline" size="sm" />}>
          ...
          <div className="grid gap-2 rounded-lg border border-border bg-background px-3 py-2.5">
            <div>
              <span className="text-xs font-semibold text-muted-foreground">{t("skills.targetRemoval.skill")}</span>
              <p className="mt-0.5 break-words text-sm font-medium leading-5">{removal.skillName}</p>
            </div>
            <div>
              <span className="text-xs font-semibold text-muted-foreground">{t("skills.targetRemoval.target")}</span>
              <p className="mt-0.5 break-words text-sm font-medium leading-5">{targetName}</p>
              <p className="mt-0.5 break-all font-mono text-xs leading-5 text-muted-foreground">{removal.targetPath}</p>
            </div>
          </div>

          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div role="group" aria-label={t("skills.targetRemoval.options")} className="flex flex-wrap items-center gap-x-4 gap-y-2">
              {canRemoveTargetPreference ? (
                <div className="flex items-center gap-2 text-sm font-medium">
                  <Checkbox checked={removeTarget} disabled={isExecuting} aria-label={t("skills.targetRemoval.removeTarget")} onCheckedChange={(nextChecked) => setRemoveTarget(Boolean(nextChecked))} />
                  <span>{t("skills.targetRemoval.removeTarget")}</span>
                </div>
              ) : null}
              <div className="flex items-center gap-2 text-sm font-medium">
                <Checkbox checked={deleteInstalledFiles} disabled={isExecuting} aria-label={t("skills.targetRemoval.deleteSkillFiles")} onCheckedChange={(nextChecked) => setDeleteInstalledFiles(Boolean(nextChecked))} />
...
const DistributionConfirmationDialog = ({
  ...
  const hasRuntimeOverwriteSelection = items.some((item) => {
    return canOverwriteRuntimeConflict(executionItemStatuses[item.id]) ? runtimeOverwriteResolutions[item.id] : false;
  });
        ...
        <DialogPopup>
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              ...
              <DialogDescription>{t("skills.distribution.confirmDescription", { count: items.length })}</DialogDescription>
            </div>
            <DialogClose disabled={isExecuting} render={<Button type="button" variant="outline" size="sm" />}>
              {t("skills.actions.close")}
            </DialogClose>
          ...
          <div className="grid gap-2">
            {items.map((item) => {
              const resolution = conflictResolutions[item.id] ?? item.defaultResolution ?? "overwrite";
              ...
              return (
                <div key={item.id} data-testid={`distribution-preview-item-${item.id}`} className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-3 gap-y-1 rounded-lg border border-border bg-background px-3 py-2">
                  <strong className="col-start-1 row-start-1 block min-w-0 truncate text-sm leading-5" title={item.targetName}>
                    {item.targetName}
                  </strong>
                  <p className="col-start-1 row-start-2 min-w-0 truncate font-mono text-xs leading-4 text-muted-foreground" title={item.targetPath}>
                    {item.targetPath}
                  </p>
                  <div data-testid={`distribution-status-slot-${item.id}`} className="col-start-2 row-start-1 flex h-6 w-24 items-center justify-end gap-2">
                    {executionStatus ? <DistributionItemExecutionIcon name={item.targetName} status={executionStatus} /> : <span aria-hidden="true" className="size-6" />}
                    <Badge variant={getDistributionActionBadgeVariant(item.action)}>{t(`skills.distribution.actions.${item.action}`)}</Badge>
                  </div>
                  <div data-testid={`runtime-overwrite-slot-${item.id}`} className="col-start-2 row-start-2 flex h-5 w-24 items-center justify-end">
                    {showRuntimeOverwriteControl ? (
                      <label className="flex h-5 cursor-pointer items-center gap-1.5 whitespace-nowrap text-xs font-medium text-red-500">
                        <Checkbox
                          ...
                          disabled={isExecuting}
                          onCheckedChange={(nextChecked) => onRuntimeOverwriteResolutionChange(item.id, nextChecked)}
                        />
                        {t("skills.distribution.runtimeOverwrite")}
                      </label>
                    ) : null}
                  </div>
                  {item.reason ? <p className="col-span-2 row-start-3 text-xs leading-5 text-muted-foreground">{item.reason}</p> : null}
                  {item.action === "conflict" ? (
                    <label className="col-span-2 grid gap-1 text-xs font-semibold text-muted-foreground">
                      ...
                      <select
                        ...
                        disabled={isExecuting}
                        value={resolution}
                        onChange={(event) => onConflictResolutionChange(item.id, event.currentTarget.value === "skip" ? "skip" : "overwrite")}
                      >
                        {allowedResolutions.includes("overwrite") ? <option value="overwrite">{t("skills.distribution.resolutions.overwrite")}</option> : null}
                        {allowedResolutions.includes("skip") ? <option value="skip">{t("skills.distribution.resolutions.skip")}</option> : null}
                      </select>
                    </label>
                  ) : null}
                </div>
          ...
          <div className="mt-5 flex justify-end gap-2">
            ...
            <Button type="button" disabled={isExecuting} onClick={executionFinished && !hasRuntimeOverwriteSelection ? onClose : onConfirm}>
              {isExecuting ? (
                <>
                  <LoaderCircle aria-hidden="true" className="animate-spin" />
...
const DistributionItemExecutionIcon = ({ name, status }: { name: string; status: SkillsPageState["distributionExecutionItemStatuses"][string] }) => {
  const { t } = useTranslation();

  if (status.status === "loading") {
    return (
      <span role="status" aria-label={t("skills.distribution.itemDistributing", { name })} className="flex size-6 items-center justify-center rounded-md text-primary">
        <LoaderCircle aria-hidden="true" className="size-3.5 animate-spin" />
      </span>
    );
  }

  const isErrorResult = status.result === "blocked" || status.result === "conflict" || status.result === "failed";
  const Icon = getDistributionResultIcon(status.result);
  const statusElement = (
    <span role="status" tabIndex={isErrorResult ? 0 : undefined} aria-label={t(`skills.distribution.itemResults.${status.result}`, { name })} className={cn("flex size-6 items-center justify-center rounded-md text-muted-foreground outline-none focus-visible:ring-3 focus-visible:ring-ring/50", (status.result === "installed" || status.result === "updated") && "text-primary", isErrorResult && "text-destructive")}>
...
const canOverwriteRuntimeConflict = (status: SkillsPageState["distributionExecutionItemStatuses"][string] | undefined): boolean => {
  return status?.status === "result" && status.result === "conflict";
};

const getDistributionResultIcon = (result: NonNullable<SkillsPageState["distributionExecutionItemStatuses"][string]["result"]>) => {
...
const toFriendlyDistributionErrorMessage = (errorMessage: string | null, t: ReturnType<typeof useTranslation>["t"]): string => {
  ...
  if (normalizedMessage.includes("eperm") || normalizedMessage.includes("eacces") || normalizedMessage.includes("operation not permitted") || normalizedMessage.includes("permission denied")) {
    return t("skills.distribution.errors.permission");
  }

  if (normalizedMessage.includes("enoent") || normalizedMessage.includes("no such file or directory") || normalizedMessage.includes("source skill directory is missing")) {
    return t("skills.distribution.errors.missingPath");
  }

  if (normalizedMessage.includes("enospc") || normalizedMessage.includes("no space left on device")) {
...
const getDistributionActionBadgeVariant = (action: "blocked" | "install" | "update" | "skip" | "conflict"): React.ComponentProps<typeof Badge>["variant"] => {
...
const SkillDescription = ({ description }: { description: string }) => {
  ...
  const descriptionElement = (
    <p ref={descriptionRef} tabIndex={isOverflowing ? 0 : undefined} className="mt-3 line-clamp-5 max-w-full break-all text-sm leading-6 text-muted-foreground outline-none focus-visible:ring-3 focus-visible:ring-ring/50">
      {description}
    </p>
  );
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="left" align="center" className="max-w-sm whitespace-normal break-all leading-5">
        {description}
      </TooltipContent>
    </Tooltip>
  );
