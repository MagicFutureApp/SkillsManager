...
import { AlertTriangle, CircleHelpIcon, Copy, Database, ExternalLink, FolderOpen, GitBranch, KeyRound, PackageCheck, RotateCcw } from "lucide-react";
...
const getVisibleSettingsNavigationItems = (isDataResetVisible: boolean) => settingsNavigationItems.filter((item) => isDataResetVisible || item.href !== DATA_RESET_NAVIGATION_HREF);

const splitHashHistoryLocationHash = (hash: string) => {
  const nestedHashIndex = hash.indexOf("#", 1);

  if (nestedHashIndex < 0) {
    return hash.startsWith("#/") ? { routeHash: hash, settingsNavigationHref: "" } : { routeHash: "", settingsNavigationHref: hash };
  }
...
const getSettingsNavigationHrefFromHash = (hash: string, isDataResetVisible: boolean): SettingsNavigationHref => {
  const { settingsNavigationHref } = splitHashHistoryLocationHash(hash);

  return getVisibleSettingsNavigationItems(isDataResetVisible).find((item) => item.href === settingsNavigationHref)?.href ?? settingsNavigationItems[0].href;
};
...
export const SettingsPage = () => {
  ...
  const [activeSettingsNavigationHref, setActiveSettingsNavigationHref] = useState<SettingsNavigationHref>(() => getSettingsNavigationHrefFromHash(window.location.hash, false));
  ...
  useEffect(() => {
    const syncActiveNavigationHref = () => {
      setActiveSettingsNavigationHref(getSettingsNavigationHrefFromHash(window.location.hash, isDataResetVisible));
    };
  ...
  const navigateToSettingsSection = (event: React.MouseEvent<HTMLAnchorElement>, href: SettingsNavigationHref) => {
    event.preventDefault();
    setActiveSettingsNavigationHref(href);
    window.history.pushState(null, "", `${window.location.pathname}${window.location.search}${getSettingsNavigationLinkHref(window.location.hash, href)}`);
  ...
  const openLatestDownload = () => {
    ...
    const opening = latestRelease.downloadUrl ? window.skillsManager?.openExternalUrl?.(latestRelease.downloadUrl) : window.skillsManager?.openAppUrl?.("officialSite");
  ...
  const activeSettingsSection = (() => {
    switch (activeSettingsNavigationHref) {
      case "#github-token":
        return (
          <SettingsPanel id="github-token" icon={<KeyRound aria-hidden="true" />} title="凭证管理" description="管理 Skills Manager 访问代码托管平台所需的凭证。">
            <CredentialProviderBlock
              ...
              titleAction={
                <Tooltip>
                  <TooltipTrigger render={<Button type="button" variant="ghost" size="icon-xs" aria-label="查看 GitHub token 创建帮助" onClick={openGitHubTokenHelp} />}>
                    <CircleHelpIcon />
                  </TooltipTrigger>
            ...
            >
              <div className="grid max-w-2xl gap-4">
                <Field>
                  ...
                  <Input type="password" value={githubToken} placeholder={settings.github.hasToken ? "********" : "请输入 GitHub Token"} disabled={isSaving} onValueChange={setGithubToken} />
                </Field>
                ...
                {status === "saved" ? <p className="text-sm text-muted-foreground">已保存 GitHub token。</p> : null}
      ...
      case "#skill-distribution":
        return (
          <SettingsPanel id="skill-distribution" icon={<PackageCheck aria-hidden="true" />} title="技能分发" description="控制来源同步完成后，是否自动分发到已经在 Skills/Targets 中设置的目标目录。" action={<Badge variant={autoDistributeOnSync ? "secondary" : "outline"}>{autoDistributeOnSync ? "已开启" : "已关闭"}</Badge>}>
            <div className="grid max-w-2xl gap-3">
              <label className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 rounded-lg border border-border bg-muted/40 p-3">
                <span className="min-w-0">
                  <span id="skill-distribution-auto-label" className="block text-sm font-semibold text-foreground">
                    同步后自动分发到已设置目标
                  </span>
                  <span className="mt-1 block text-sm leading-6 text-muted-foreground">开启后，从来源同步完成后，各个技能会自动分发到已选择的目标目录。</span>
                </span>
                <Switch aria-labelledby="skill-distribution-auto-label" checked={autoDistributeOnSync} disabled={isSavingDistribution} onCheckedChange={updateAutoDistributeOnSync} />
              </label>
              {distributionStatus === "saved" ? <p className="text-sm text-muted-foreground">已保存分发设置。</p> : null}
            </div>
          </SettingsPanel>
        );
      case "#local-storage":
        return (
          <SettingsPanel id="local-storage" icon={<Database aria-hidden="true" />} title="本地存储" description="查看 Skills Manager 的本地缓存根目录和 SQLite 数据库文件路径。">
            <div className="grid gap-3">
              <StoragePathRow icon={<FolderOpen aria-hidden="true" />} label="本地缓存总路径" value={storagePaths?.localCachePath ?? (storageStatus === "loading" ? "读取中..." : "--")} copyLabel="复制路径" disabled={!storagePaths?.localCachePath} onCopy={() => storagePaths?.localCachePath && copyPath(storagePaths.localCachePath)} />
              <StoragePathRow icon={<Database aria-hidden="true" />} label="本地数据库路径" value={storagePaths?.databasePath ?? (storageStatus === "loading" ? "读取中..." : "--")} copyLabel="复制路径" disabled={!storagePaths?.databasePath} onCopy={() => storagePaths?.databasePath && copyPath(storagePaths.databasePath)} />
            </div>
          </SettingsPanel>
        );
      case DATA_RESET_NAVIGATION_HREF:
        ...
        return (
          <section className="grid scroll-mt-6 gap-4 rounded-xl border border-destructive/25 bg-destructive/5 p-4" aria-labelledby="settings-data-reset">
            <div>
              ...
              <p className="mt-4 text-sm leading-6 text-muted-foreground">重建数据库会清空本地索引、来源、Skills 和应用设置。已安装到 agent 目标目录的文件不会被删除。</p>
            </div>

            <Button type="button" variant="destructive" disabled={isResetting} onClick={() => setIsResetDialogOpen(true)}>
              <RotateCcw aria-hidden="true" />
              重建本地数据库
            </Button>
            {storageStatus === "reset" ? <p className="text-sm text-muted-foreground">本地数据库已重建。</p> : null}
            {storageError ? <p className="text-sm text-destructive">{storageError}</p> : null}
          </section>
        );
      case "#settings-about":
        return (
          <section id="settings-about" className="flex min-h-[calc(100svh-100px)] scroll-mt-6 items-center justify-center text-center" aria-labelledby="settings-about-heading">
            ...
            <div className="flex flex-col items-center justify-center">
              ...
              <div className="mt-4 text-center">
                <button type="button" onClick={openOfficialSite} aria-label="访问 Skills Manager 官方网站" className="inline-flex items-center gap-1.5 rounded-sm text-lg font-semibold text-foreground outline-none transition-colors hover:text-primary focus-visible:text-primary">
                  Skills Manager
                  <ExternalLink className="size-4 text-muted-foreground" aria-hidden="true" />
                </button>
                <p className="mt-1 text-xs font-medium text-muted-foreground">Sync and distribute agent skills</p>
              </div>
              <p className="mt-5 text-sm font-medium text-muted-foreground">版本 {appInfo?.version ? `v${appInfo.version}` : "--"}</p>
              <LatestReleaseHint isChecking={isCheckingLatestRelease} latestRelease={latestRelease} currentVersion={appInfo?.version} onOpenDownload={openLatestDownload} />
              {officialSiteError ? <p className="mt-2 text-sm text-destructive">{officialSiteError}</p> : null}
            </div>
          </section>
        );
    }
  ...
  return (
    <div className="grid h-full grid-cols-[248px_minmax(0,1fr)] overflow-hidden bg-background">
      <aside className="sticky top-0 flex h-[calc(100svh-44px)] flex-col overflow-y-auto border-r border-border bg-card px-4 py-5" aria-label="设置页面侧边栏">
        <div className="mb-5">
        ...
        <nav className="grid gap-1" aria-label="设置页面导航">
          {getVisibleSettingsNavigationItems(isDataResetVisible).map((item) => {
            ...
            return (
              <a key={item.href} href={getSettingsNavigationLinkHref(window.location.hash, item.href)} className={["rounded-lg px-3 py-2 text-sm font-medium outline-none transition-colors focus-visible:ring-3 focus-visible:ring-ring/50", isActive ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"].join(" ")} aria-current={isActive ? "location" : undefined} onClick={(event) => navigateToSettingsSection(event, item.href)}>
                {item.label}
              </a>
            );
          })}
      ...
      <main ref={settingsContentRef} className="h-[calc(100svh-44px)] min-w-0 overflow-y-auto p-7" aria-label="设置内容">
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>会清空本地索引、来源、Skills 和应用设置，但不会删除已安装到 agent 目标目录的文件，也不会清空本地缓存目录。</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isResetting}>取消</AlertDialogCancel>
            <AlertDialogAction type="button" variant="destructive" disabled={isResetting} onClick={confirmResetLocalDatabase}>
              <RotateCcw aria-hidden="true" />
              确认重建
            </AlertDialogAction>
          </AlertDialogFooter>
    ...
    <section className="rounded-xl border border-border bg-card p-4" aria-labelledby={id}>
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="inline-flex size-4 items-center justify-center text-muted-foreground [&_svg]:size-4">{icon}</span>
...
const CredentialProviderBlock = ({
  ...
  return (
    <section className="rounded-lg border border-border bg-muted/40 p-3" aria-labelledby={id}>
      <div className="flex items-center gap-2">
        <span className="inline-flex size-4 items-center justify-center text-muted-foreground [&_svg]:size-4">{icon}</span>
...
const StoragePathRow = ({ copyLabel, disabled, icon, label, value, onCopy }: { copyLabel: string; disabled: boolean; icon: React.ReactNode; label: string; value: string; onCopy: () => void }) => {
  return (
    <div className="grid gap-3 rounded-lg border border-border bg-muted/40 p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
      <div className="min-w-0">
        <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">
          <span className="inline-flex size-4 items-center justify-center [&_svg]:size-4">{icon}</span>
          {label}
        </div>
