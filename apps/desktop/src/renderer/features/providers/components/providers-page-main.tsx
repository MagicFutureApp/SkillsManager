...
export const ProvidersPageMain = () => {
  ...
  return (
    <>
      <header className="mb-6">
        <div className="flex items-center justify-between gap-4 max-[860px]:items-start">
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">{t("providers.description")}</p>
          </div>
      ...
      <div className="mt-5">
        <ProviderList actionLabel={(provider) => (provider.connected ? t("providers.actions.disconnect") : t("providers.actions.connect"))} authHeader={t("providers.table.auth")} connectionHeader={t("providers.table.connection")} emptyMessage={t("providers.empty")} providerHeader={t("providers.table.provider")} providers={page.visibleProviders} selectedProviderId={page.selectedProviderId} statusHeader={t("providers.table.status")} onSelectProvider={page.setSelectedProviderId} onToggleConnection={page.toggleConnection} />
      </div>
    </>
  );
};
