...
export const RepositoryList = ({ checkedIds, copy, pagination, repositorySyncStates, repositories, selectedRepositoryId, visibleAllChecked, visibleSomeChecked, onPageChange, onSelectAllVisible, onSelectRepository, onSyncRepository, onToggleChecked, onToggleEnabled }: RepositoryListProps) => {
  return (
    <DataTableFixed>
        ...
        <DataTableRow>
          <DataTableHead className={repositoryTableColumns.select}>
            <span className="grid place-items-center">
              <Checkbox checked={visibleAllChecked} indeterminate={visibleSomeChecked && !visibleAllChecked} disabled={!repositories.length} aria-label={copy.selectAll} onCheckedChange={onSelectAllVisible} />
            </span>
          </DataTableHead>
          ...
          repositories.map((repository) => (
            ...
            >
              <DataTableCell className={repositoryTableColumns.select}>
                <span className="grid place-items-center">
                  <Checkbox checked={checkedIds.has(repository.id)} aria-label={copy.selectRepository(repository.name)} onCheckedChange={(checked) => onToggleChecked(repository.id, checked)} />
                </span>
              </DataTableCell>
              <DataTableCell className="min-w-0">
                <Button type="button" variant="ghost" className="grid h-auto min-w-0 justify-start gap-1 px-0 py-0 text-left font-normal hover:bg-transparent focus-visible:ring-3 focus-visible:ring-ring/50" aria-label={repository.name} aria-selected={repository.id === selectedRepositoryId} onClick={() => onSelectRepository(repository.id)}>
                  <span className="truncate text-sm font-semibold">{repository.name}</span>
                </Button>
              </DataTableCell>
              <DataTableCell className={`${repositoryTableColumns.provider} text-sm max-[820px]:hidden`}>{repository.provider}</DataTableCell>
              <DataTableCell className={`${repositoryTableColumns.status} max-[820px]:hidden`}>
                <RepositoryStatusPill status={repository.status} />
              </DataTableCell>
              <DataTableCell className={`${repositoryTableColumns.skills} font-mono text-sm max-[820px]:hidden`}>{repository.skillUnits}</DataTableCell>
              <DataTableCell className={repositoryTableColumns.sync}>
                <RepositorySyncIndicator repository={repository} state={repositorySyncStates[repository.id]} onSyncRepository={onSyncRepository} />
              </DataTableCell>
              <DataTableCell className={repositoryTableColumns.actions}>
                <Switch checked={repository.enabled} aria-label={copy.toggleEnabled(repository.name)} onCheckedChange={() => onToggleEnabled(repository.id)} />
              </DataTableCell>
            </DataTableRow>
          ))
        )}
      ...
      <DataTablePaginationFooter colSpan={7} labelKeyPrefix="repositories.pagination" onPageChange={onPageChange} pagination={pagination} />
    </DataTableFixed>
  );
};

const RepositorySyncIndicator = ({ repository, state, onSyncRepository }: { repository: RepositoryViewModel; state?: RepositorySyncState; onSyncRepository: (repositoryId: string) => void }) => {
  ...
  return (
    <Tooltip>
      <TooltipTrigger
        ...
        type="button"
        className={cn("grid size-7 place-items-center rounded-md text-muted-foreground outline-none transition-colors hover:bg-muted focus-visible:ring-3 focus-visible:ring-ring/50", syncClassNameByStatus[status])}
        onClick={(event) => {
...
const resolveSyncIndicatorState = (repository: RepositoryViewModel, state?: RepositorySyncState): { message: string; status: RepositorySyncState["status"] | "idle" } => {
  if (state) {
    return state;
  }
  ...
  if (repository.lastSync.status === "interrupted") {
    return {
      message: repository.lastSync.errorMessage ? `最后一次同步被中断。${repository.lastSync.errorMessage}` : "最后一次同步被中断。",
      status: "failed"
    };
  }
  ...
  if (repository.lastSync.status === "failed") {
    return {
      message: repository.lastSync.errorMessage ? `最后一次同步失败。${repository.lastSync.errorMessage}` : "最后一次同步失败。",
      status: "failed"
    };
  }
...
const buildLastSuccessfulSyncMessage = (repository: RepositoryViewModel): string => {
  const skillSummary = repository.skillUnits > 0 ? `最后一次同步成功。已入库 ${repository.skillUnits} 个 Skills。` : "最后一次同步成功。未发现可入库的 Skills。";
