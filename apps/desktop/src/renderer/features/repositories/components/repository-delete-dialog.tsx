...
export const RepositoryDeleteDialog = ({ copy, error, isDeleting, isLoading, open, preview, onClose, onConfirm }: RepositoryDeleteDialogProps) => {
  if (!open) {
    return null;
  }
      ...
      <DialogPortal>
        ...
        <DialogPopup>
                ...
                {preview.skills.length ? (
                  <ul className="mt-2 max-h-56 overflow-auto rounded-lg border border-border">
                    {preview.skills.map((skill) => (
                      <li key={skill.id} className="border-b border-border px-3 py-2 last:border-b-0">
                        <span className="block text-sm font-medium">{skill.name}</span>
                        <span className="mt-1 block break-words font-mono text-xs text-muted-foreground">{skill.entryPath}</span>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className="mt-2 rounded-lg border border-border bg-muted/40 p-3 text-sm text-muted-foreground">{copy.emptySkills}</p>
                )}
              </div>
            </div>
          ) : null}
          ...
          <div className="mt-5 flex justify-end gap-2">
            <DialogClose render={<Button type="button" variant="outline" disabled={isDeleting || isLoading} />}>{copy.cancel}</DialogClose>
            <Button type="button" variant="destructive" disabled={!preview || isDeleting || isLoading} onClick={onConfirm}>
              {copy.confirm}
            </Button>
          </div>
        </DialogPopup>
