  ...
  return (
    <div className="w-full max-w-xl text-left">
      <div className="overflow-hidden rounded-lg border border-zinc-800 bg-zinc-900 shadow-sm">
        <div className="flex items-center justify-between border-b border-zinc-800 px-3.5 py-2">
          <span className="font-mono text-xs text-zinc-400">macOS · Homebrew</span>
          <button
            type="button"
            onClick={handleCopy}
            aria-label="复制 Homebrew 安装命令"
            className="inline-flex items-center gap-1 rounded border border-zinc-700 bg-zinc-800 px-2 py-1 text-xs text-zinc-300 transition-colors hover:bg-zinc-700 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-zinc-400"
          >
            {copied ? <Check className="size-3.5" /> : <Copy className="size-3.5" />}
            <span>{copied ? "已复制" : "复制"}</span>
          </button>
        </div>
        <pre className="overflow-x-auto px-3.5 py-3 font-mono text-xs leading-relaxed text-zinc-100">
