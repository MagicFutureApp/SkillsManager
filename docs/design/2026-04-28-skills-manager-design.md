...
当前 HTML 原型以桌面端线框为准，导航入口和页面内容已按当前验证重点收缩：

- `Diagnostics`：暂时隐藏入口，页面文件保留，后续需要诊断流时可恢复。
- `Settings`：保留设置主内容；删除右侧边栏、Grid、统计模块、搜索/筛选模块，以及“强制重新扫描”“新增配置”入口。
- `Skills`：Grid 第一列增加批量选择 CheckBox，表头支持全选/半选；顶部保留“同步”和“新增技能”，删除“复制技能 ID”，隐藏“预览计划”。
- `Targets`：Grid 保留目标、类型、启用和操作列；删除“新增目标”入口；行操作提供删除并二次确认。右侧信息区删除“目标配置”和“最近校验”，安全检查只显示“路径存在”和“写权限”。
- `Distribution`：删除中间统计区域和中间右上的“重新计算”“新建计划”入口。右侧详情栏删除“复制计划摘要”“编辑计划”“执行计划”按钮，删除“计划配置”和“执行日志”，并将“Dry-run 条目”显示为“详细”。

这些调整只描述当前 HTML 原型的可见 UI，不改变第 6 节的数据模型或第 13 节实现方案。
