...
1. **推荐：右键打开**
   在 Finder 中右键（或按住 Control 单击）`Skills Manager.app`，选择「打开」，然后在弹窗中再次点击「打开」。本次放行后，后续双击即可正常启动。

2. **命令行移除隔离标记**
   如果右键打开仍被拦截，可以在终端移除该 app 的隔离标记（把路径替换成实际安装位置）：
...
> 提示：不要从不明来源下载安装包。建议只从[官方网站](https://sk.magicfuture.app)或 [GitHub Releases](https://github.com/MagicFutureApp/SkillsManager/releases/latest) 获取。
...
1. 在 SmartScreen 警告窗口点击「**更多信息**」。
2. 窗口下方会出现「**仍要运行**」按钮，点击即可继续安装。
...
- 使用 `apt install ./<file>.deb` 安装时不会校验签名，正常安装即可。
- 双击通过发行版图形包管理器安装同样不受签名影响。
